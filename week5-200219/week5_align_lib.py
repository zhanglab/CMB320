    ...
    for i in xrange(1, len(x)+1):
        for j in xrange(1, len(y)+1):
            ...
            if A[i][j] == A[i][j-1] + score.gap:
                D[i][j] += 'u'
            if A[i][j] == A[i-1][j] + score.gap:
                D[i][j] += 'l'
            if A[i][j] == A[i-1][j-1] + score.matchchar(x[i-1], y[j-1]):
                D[i][j] += 'd'
...
def global_align(x, y, score=ScoreParam(10, -2, -7, -15)):
    ...
    # create two zero-filled matrices, one for storing score, another for
    # storing the direction for tracing back the optimal scores
    A = make_matrix(len(x) + 1, len(y) + 1)
    D = make_matrix(len(x) + 1, len(y) + 1)

    for i in xrange(1, len(x)+1):
        A[i][0] = score.gap_start + (i-1) * score.gap
        D[i][0] = 'l'

    for i in xrange(1, len(y)+1):
        A[0][i] = score.gap_start + (i-1) * score.gap
        D[0][i] = 'u'
